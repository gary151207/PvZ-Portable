...
小游戏 `GAMEMODE_CHALLENGE_CRICKET`：第 3 行单行草坪、无经济（`mSunMoney = 0`）、
双方全随机、玩家不参战；分胜负后就地重掷随机量、直接开下一场。
**斗蛐蛐 2**（`GAMEMODE_CHALLENGE_CRICKET_2`）是另一个模式（自选卡 + 自选出怪的录制沙盒，
差异表见 `docs/superpowers/specs/2026-09-17-cricket-fight-2-design.md`）。

### 语言

**植物池 (Plant Pool)**：
斗蛐蛐每场从池中等概率抽 **5 株**，摆在`第 3 行第 1-5 列`。池子是**全量自动收集**的
（`Board::SetupCricketFight` 遍历 `SEED_PEASHOOTER..NUM_SEED_TYPES`），只排除 7 种
（`gCricketBannedSeeds`）：南瓜头、花盆、玉米加农炮、模仿者、爆炸坚果、左向双发射手，
以及**禅境花园幼苗**（`SEED_SPROUT`，站桩时什么都不做，白白占掉 5 个位置之一）。
**模组新增植物不需要任何登记就会自动参赛**——含没有卡片的隐藏形态（寒冰/火焰/三线机枪射手、
魅惑大喷菇）；要调整只动那份排除名单。
_避免_：可选植物、植物阵容表

**僵尸池 (Zombie Pool)**：
与植物池相反，僵尸是**写死的名单**：`gCricketZombiePool`（24 种常规僵尸，含 6 种僵尸植物头）
再并上 `gTravelZombieDefs`（旅行专属僵尸等概率参战，但**每场至多 1 只**，第二次抽中就地
改成普通僵尸）。每场固定 1 波 6 只。
_避免_：出怪表、随机僵尸

**战绩统计 (Cricket Stats)**：
每场结果按**枚举序号**记进 `userdata/cricket_stats.txt`（`SEED_<n>` / `ZOMBIE_<n>` 与
`__MATCH_COUNT__`），关内按 **Tab** 开面板看胜率。面板**只列有出场记录的类型**，
所以刚加入的植物要等它出场过一次才会出现在榜单上（不是"没被加入池子"）。
_避免_：胜率表、对战记录

### 关系

- 斗蛐蛐是**夜间草坪**（`PickBackground` → `BACKGROUND_2_NIGHT`）：蘑菇类（大喷菇群、
  魅惑大喷菇、孢子菇等）不会睡着，所以**所有**植物都能直接参战（这也是植物池敢全量收集的前提）
- 斗蛐蛐 1 **没有选卡器、卡槽是空的**（阳光也是 0）：想自己布阵请用斗蛐蛐 2
- 第 3 行那辆小推车是**判负边界**；僵尸走到最左边即植物方失败，随后自动重开
- **植物池随枚举长大**，僵尸池随名单长大：新增植物记得看一眼 `check-cricket-pool.ps1`
