...
| 机制 | 规则 | 实现落点 |
|---|---|---|
| 自动回血 | 每 5 秒（500 tick）回复 200 HP，不超过 32000 上限 | `Plant::Update` 巨大坚果分支 + `Plant.mGiantRegenCountdown` |
| 免疫碾压秒杀 | 被碾压/砸击不判死，改为**扣 500 HP**；死亡判定照常（HP≤0 才消失） | `Zombie::SquishAllInSquare` 巨大坚果分支 |
| 车类僵尸击退 | 若碾压来源为**车类僵尸**（铲雪车 ZOMBIE_ZAMBONI、投石车 ZOMBIE_CATAPULT），将其向后（右）**击退一格**（≈80px，退出受击矩形避免每帧重复触发） | 同上分支（非车类如巨人砸击只扣血） |
| 免疫小丑爆炸秒杀 | 小丑僵尸/毁灭菇头爆炸（`Board::KillAllPlantsInRadius`）波及巨大坚果时不再秒杀：若爆炸圈内有巨大坚果，**全场所有巨大坚果平均分摊合计 2000 伤害**（圈外也参与分担；HP≤0 者照常消失） | `Board::KillAllPlantsInRadius` 预扫描巨大坚果 |

设计要点：
- 所有特殊处理只在**种植形态**（非保龄球滚球）下有意义；爆炸/碾压路径本身在保龄球关不产生，
  无需模式逃生门。
- 车类击退靠位移退出受击矩形自限频（回位后再次接触才会再扣血/再击退），不需新增每僵尸状态。
- 巨人砸击（`SquishAllInSquare` CHEW 路径）同样按碾压规则扣 500 但不击退（非车类）。

### 入口可见性排查结论

"小游戏里看不到旅行体验"根因 = **运行的是旧构建**（如 `dist/` 打包目录里 8-30 的
...
10. 增补机制：受伤后每 5 秒回血 200（观察血条/裂纹回退）；铲雪车/投石车碾过不掉判死而扣 500
    并被向后击退一格（同一行重新碾压会再次触发）；巨人砸击同样扣 500（不击退）；
    小丑爆炸波及巨大坚果不死：场上有 1 只扣 2000、2 只各扣 1000（圈外也分担）；
    普通坚果/其它植物对小丑爆炸与碾压行为不变。
